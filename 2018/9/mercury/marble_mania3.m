:- module marble_mania3.
:- interface.
:- import_module zipper, array.
...
:- inst game for game/0
    == bound(game(ground, ground, zipper_skel(ground))).
...
:- implementation.
:- import_module circular_zipper, int.

start(N) = game(1, array.init(N, 0), zipper.init(0)).

scoring(G) :- G^next_marble mod 23 = 0.

:- pred previous(int::in, game::game_di, game::game_uo) is det.
previous(N, game(R, S, !.B), game(R, S, !:B)) :- circle_previous(N, !B).

:- pred next(game::game_di, game::game_uo) is det.
next(game(R, S, !.B), game(R, S, !:B)) :- circle_next(!B).

:- pred insert(game::game_di, game::game_uo) is det.
insert(game(R, S, !.B), game(R, S, !:B)) :- insert_right(R, !B).

:- pred delete(game::game_di, game::game_uo) is det.
delete(game(R, S, !.B), game(R, S, !:B)) :-
    circle_delete_right(!B).

:- func get(game) = int.
get(game(_, _, B)) = R :- focus(B, R).
