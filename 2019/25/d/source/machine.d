...
struct Machine {
    ...
    BigInt at(int param, Mode[3] modes) in (param > 0 && param < 4) {
    ...
    ref BigInt write(int param, Mode[3] modes) in (param > 0 && param < 4) {
        ...
        while (true) {
            ...
            final switch (opmode.opcode) {
                ...
                case IntOp.Put:
                    ...
                    if (output == Tid.init) write(at(1, m));
                    else send(output, at(1, m));
                    break;
            }
...
BigInt amplify(int[] phase, immutable int[] mem) in (phase.length > 1) {
    Tid[] machines;
